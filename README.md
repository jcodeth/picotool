...
The AES key can either be provided as a 1024 bit (128 byte) .bin file containing 4-way shares of each word of the 256 bit AES key to be used for encryption, or as a 256 bit (32 byte) .bin file containing the 256 bit AES key.
...
When passing a 256 bit AES key (as opposed to a 1028 bit 4-way AES key share), picotool will generate a random key share to match that key, by generating 3 random shares (A, B & C) and calculating a 4th share (D) to match the provided key.
